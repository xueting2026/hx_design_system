...
1. **权威源**：原型引用的样式必须指向本 skill 的权威 `references/ui.css`。
...
> 上述校验由 `scripts/verify-prototype.mjs` 自动执行：`node skills/xinxiaodi-design/scripts/verify-prototype.mjs <原型HTML绝对路径>`（默认 `--ds references`）。
