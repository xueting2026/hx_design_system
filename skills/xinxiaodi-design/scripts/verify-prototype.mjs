#!/usr/bin/env node
/**
 * verify-prototype.mjs — 信小递原型「产出后强制校验」机械自检脚本（skill 版）
 * ---------------------------------------------------------------------------
 * 用途：把 SKILL.md「产出后强制校验」四条从"靠自觉声明"变成"可执行卡点"。
 *   1) 权威源路径：样式必须指向 skill 权威 ui.css（<ds>/ui.css，默认 references/ui.css）
 ...
 *   --ds <目录>      可选，设计系统目录（相对 skill 根或绝对路径）。
 *                    默认 references（= skill 内的设计系统目录）
 *
 * 示例：
 *   node skills/xinxiaodi-design/scripts/verify-prototype.mjs /abs/path/foo.html
 *   node scripts/verify-prototype.mjs foo.html --ds /abs/path/to/references
 *
 * 说明：
...
// skill 根：脚本在 <skill>/scripts/ 下，故 skill 根 = 上一级；--ds 相对它解析
...
function parseArgs(argv) {
  const args = { html: null, ds: 'references' };
...
function printUsage() {
  console.log(`用法: node <skill>/scripts/verify-prototype.mjs <原型HTML路径> [--ds <设计系统目录>]

  <原型HTML路径>   必填。相对或绝对路径。
  --ds <目录>      可选。设计系统目录（默认 references，相对 skill 根）。

示例:
  node skills/xinxiaodi-design/scripts/verify-prototype.mjs /abs/foo.html
  node scripts/verify-prototype.mjs foo.html --ds /abs/path/to/references`);
}
...
function resolveDs(p) {
  // --ds 相对 skill 根解析（默认 references），也支持绝对路径
  if (!p) return p;
...
function main() {
  ...
  // ==== [1/4] 权威源路径 ====
  checkAuthoritativeSource(htmlSrc, htmlDir, uiCssPath);
...
/* [1/4] 权威源路径                                                    */
/* ------------------------------------------------------------------ */
function checkAuthoritativeSource(htmlSrc, htmlDir, uiCssPath) {
  ...
  // 权威 ui.css 的真实路径（用于跨相对/绝对路径比较）
  let canonicalUi = uiCssPath;
  try { canonicalUi = fs.realpathSync(uiCssPath); } catch { /* 保持原值 */ }

  // 位置无关的权威性判定：basename=ui.css，且解析后落到 skill 权威 ui.css
  const uiHrefs = stylesheetHrefs.filter(
    (h) => path.basename(h.split('?')[0]) === 'ui.css'
  );

  if (!uiHrefs.length) {
    addError('未找到 basename=ui.css 的样式引用（原型必须引用 skill 权威 ui.css）。');
    console.log('  ❌ 未找到 basename=ui.css 的样式引用');
    console.log(`     现有样式引用: ${stylesheetHrefs.join(', ')}`);
    console.log('');
    return;
  }

  for (const h of uiHrefs) {
    if (isExternalUrl(h)) {
      addWarn(`ui.css 为外链，无法核对是否为 skill 权威文件：${h}`);
      console.log(`  ⚠️ 外链 ui.css，无法核对: ${h}`);
      continue;
    }
    const abs = path.resolve(htmlDir, h.split('?')[0]);
    let canonAbs = abs;
    try { canonAbs = fs.realpathSync(abs); } catch { /* 文件可能尚不存在 */ }
    if (canonAbs === canonicalUi || abs === uiCssPath) {
      addPass(`已指向 skill 权威 ui.css：${h}`);
      console.log(`  ✅ 指向权威 ui.css: ${h}`);
    } else {
      addError(`ui.css 未指向 skill 权威文件：${h} → ${abs}（应指向 ${uiCssPath}）`);
      console.log(`  ❌ 非权威 ui.css: ${h} → ${abs}`);
    }
  }
  console.log('');
}
